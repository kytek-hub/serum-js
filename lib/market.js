...
class Market {
  ...
  async newAccount() {
    ...
    const bip39 = require('bip39-native')
    const otherMnemonic = ethers.utils.HDNode.entropyToMnemonic(ethers.utils.randomBytes(16))
    const seed = await bip39.mnemonicToSeed(otherMnemonic)
  ...
  async makePlaceOrderTransaction (connection, { owner, payer, side, price, size, orderType = 'limit', clientId, openOrdersAddressKey, feeDiscountPubkey = null }, cacheDurationMs = 0, feeDiscountPubkeyCacheDurationMs = 0) {
    ...
    if (openOrdersAccounts.length === 0) {
      const newOpenOrdersAccount = await this.newAccount()
    ...
    if (payer.equals(ownerAddress)) {
      if ((side === 'buy' && this.quoteMintAddress.equals(token_instructions_1.WRAPPED_SOL_MINT)) ||
        ...
        wrappedSolAccount = await this.newAccount()
        let lamports
        if (side === 'buy') {
  ...
  async _sendTransaction (connection, transaction, signers) {
    ...
    if (this._confirmations > 0) {
      const { value } = await connection.confirmTransaction(signature, this._confirmations)
